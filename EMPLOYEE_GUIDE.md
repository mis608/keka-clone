...
1. Go to `/login` and use **your work email** plus the password HR gave you (in the demo it is
   `demo123`; on your own deployment it is the `EMPLOYEE_PASSWORD` value).
2. You will see a yellow bar at the top of the dashboard: *"You signed in with the shared HR
   password. Set your own…"*. Click **Set my password**.
3. Enter the password you just used, then your new one twice. Minimum 8 characters (that floor is
   `MIN_PASSWORD_LENGTH`), and it must differ from the one you used.
4. From now on **only your own password opens your account** — the shared one stops working for
   you, which is exactly the point.
5. Forgotten password? There is no self-service reset. Ask HR: they open your card and click
